              ...
              <td>
                <div class="goods">
                  ...
                  <div>
                    ...
                    <!-- 选择规格组件 -->
                    <CartSku :attrsText="goods.attrsText" :skuId="goods.skuId" @change="$event=>updateCartSku(goods.skuId,$event)" />
                  </div>
                </div>
              </td>
              <td class="tc">
                <p>&yen;{{goods.nowPrice}}</p>
                <p v-if="goods.pirce-goods.nowPrice>0">比加入时降价 <span class="red">&yen;{{goods.pirce-goods.nowPrice}}</span></p>
              </td>
              <td class="tc">
                <XtxNumbox :modelValue="goods.count" :max="goods.stock" @change="($event)=>updateCount(goods.skuId,$event)" />
              </td>
...
export default {
  name: 'XtxCartPage',
  components: { CartNone, GoodRelevant, CartSku },
  setup () {
    ...
    const deleteCart = (skuId) => {
      Confirm({ title: '温馨提示', text: '亲确认删除吗' }).then(() => {
    ...
    const checkOut = () => {
      ...
      if (!store.state.user.profile.token) {
        Confirm({ text: '下单结算需要登录，确认现在登录吗' }).then(() => {
